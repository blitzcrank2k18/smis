...
<div class="col-lg-12">
	<div class="panel panel-default">
		...
		<div class="form-group">
			...
			<select name="status" class="form-control" id="" required/>
					<option value="<?php echo $t_row['stats'] ?>"><?php
						if($t_row['stats'] == '1'){
							echo "New";
						}elseif($t_row['stats'] == '2'){
							echo "Current";
						}elseif($t_row['stats'] == '3'){
							echo "Transferee";
						}
					 ?></option>
					<?php if($t_row['stats'] != '1'){ ?><option value="1">New</option> <?php } ?>
					<?php if($t_row['stats'] != '2'){ ?><option value="2">Current</option> <?php } ?>
					<?php if($t_row['stats'] != '3'){ ?><option value="3">Transferee</option> <?php } ?>
			</select>
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<div class="col-sm-8">
			<input type="text" class="form-control" name="sec" id='sections' value="<?php echo $t_row['section'] ?>"  onkeyup="get_section()" onkeydown="get_section()" required/>
			<span id="advisorys"><div class="alert alert-success">Adviser: <?php echo $t_row['tname']; ?></div></span>
			<input type="hidden" name="adviser" id="advisers">
		</div>
		</div>
	...
	<table  class="table table-bordered">
		...
		<button type="button" onclick="new_rows()" style="float:right" id="btn_add" class="btn btn-sm btn-info"><i class="fa fa-plus"></i></button type="button" >
			...
			<div class="row">
			<div class="col-md-12">
				  <table class="table" style="width:940px;margin-left:10%" >
          ...
          <tr>
            ...
            <input style="border-bottom:0px;width:30px" class="dc" type="text" name="dc[]" value="<?php echo $att['days_classes'] ?>" >
             </td>
            <?php } ?>
    ...
    $(".p").on("keydown keyup", function() {
        calculateAVE();
						...
						jQuery("#update_grade").submit(function(e){
								...
								$.ajax({
									type: "POST",
									url: "../forms/update_forms.php?action=update_record",
									data: formData,
